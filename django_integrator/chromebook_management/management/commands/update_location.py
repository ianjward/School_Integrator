...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--fromfile',
            action='store_true',
            help='Enables Chromebooks from a CSV list of serial numbers',
        )
    ...
    def enable_chromebooks(self, file_path):
        ...
        for serial in serials:
            ...
            if serial in chromebooks.keys():
                ...
                try:
                    google_creds.chromeosdevices().action(customerId='C02vc3t7l', resourceId=google_device_id, body={"action": "reenable"}).execute()
                    print('Activated serial:', serial, google_device_id)

                except HttpError as error:
                    message = error.error_details[0]['message']

                    if message == 'Illegal device state transition.':
                        print(serial, 'already enabled')
                    else:
                        print(serial, 'HTTP error')
            else:
